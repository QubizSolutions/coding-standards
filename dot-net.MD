  ...
  * Naming should come from the *ubiquitous language*! If the business people are naming an entity Employee you shoud have in code as such rather than something different. Look below at an example:
  ``` c#
    public class Foo
    {
        void Bar1()
        {
            User employee = new User(); // class name is User but variable name is employee => not consistent
        }
        
        void Bar2()
        {
            Employee employee = new Employee(); // class name is Employee and variable name is also employee => consistent
        }
    }
    ```
